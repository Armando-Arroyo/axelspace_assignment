...
class TestExample(DbTest):
    ...
    def test_count_the_number_of_subordinates(self, conn):
        ...
        sql = """
        SELECT (SELECT COUNT(*) FROM enterprise_sales_enterprise_customers
        WHERE organizations.id = enterprise_sales_enterprise_customers.sales_organization_id) AS subordinates_count, id FROM organizations;
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql)
            actual = cur.fetchall()
    ...
    def test_segments_using_geojson_boundary(self, conn):
        ...
        WHERE ST_Within(ST_Centroid(bounds), ST_GeomFromGeoJSON('{"type": "Polygon","coordinates": [[[130.27313232421875,30.519681272749402],[131.02020263671875,30.519681272749402],[131.02020263671875,30.80909017893796],[130.27313232421875,30.80909017893796],[130.27313232421875,30.519681272749402]]], "crs":{"type":"name","properties":{"name":"EPSG:4326"}}}'))
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(sql)
            actual = cur.fetchall()
